...
$(function(){
  ...
  $(window).on('scroll', function(){
    ...
    if(window.innerWidth>640){ //pc모드일때만
  ...
  $mnu.on('click', function(evt){
    ...
    //이벤트를 강제로 발생시키는 메소드 trigger('이벤트명');
    if(window.innerWidth<=640){
      $btnGnb.trigger('click');//이벤트 강제발생, 메뉴클릭시 버튼클릭한 효과가 함께 발생하게 됨
    }
  });
...
$(function(){
  ...
  $btnNext.on('click', function(evt){
    ...
    $container.stop().animate({left:'-100%'}, 400, "easeInOutCubic", function(){
      ...
      // 무한반복 슬라이드 만들려면 left값을 -100% * idx값만큼 옮기는게 아니라 무조건 한칸만큼만 옮기는 것!!!!
      // 편도슬라이드랑 계산식이 다르다
      const $slides = $('.slides-container>li'); //li 4개

      $slides.first().appendTo($container);//마지막li로 이동
      $container.css({left:0}); //끝도 없이 바로 지나친 li를 맨 뒤에 갖다 붙이고 노출되는 li를 left:0에 놓는것!
    });
  });
  ...
  $btnPrev.on('click', function(evt){
    ...
    $slides.last().prependTo($container);//첫번째li로 이동
    $container.css({left:'-100%'}); //뒤에서 떼다붙인 첫번째 li를 앞에있는 상태로 옮김->그 뒤에 애니메이트로 움직임
...
$(function(){
  ...
  function fadeFn(){
    ...
    //슬라이드처리
    $slides.eq(oldIdx).stop().fadeOut(200);//이전슬라이드 사라지기
    $slides.eq(nowIdx).stop().fadeIn(200).css({display:'flex'})//이번 슬라이드 나타나기    
  }
